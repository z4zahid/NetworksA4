...
int rcsAccept(int socketID, struct sockaddr_in * addr) {
    ...
    sendBuf[ACK_INDEX] = ACK_BIT;
    sendBuf[ACK_NUM] = ack_num;
    sendBuf[SEQ_BIT] = seq_num;
    ...
    while (true) {
        ...
        if (receiveBuf[ACK_INDEX] == ACK_SET && receiveBuf[ACK_NUM] == seq_num + 1) {
            break;
        }
    }
    return 0;
...
int rcsConnect(int socketID, const struct sockaddr_in * addr) {
    ...
    // TODO: maybe keep sending until timeout and zero messages from server?
